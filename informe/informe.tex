...
\end{tabular}
\vspace{0.7cm}
{\normalsize\bfseries Repositorio GitHub del trabajo\par}
\vspace{0.12cm}
{\small\href{https://github.com/handdycurasi-crypto/SIS210-Semana1}{\texttt{github.com/handdycurasi-crypto/SIS210-Semana1}}\par}
\vfill
{\large Puno, Perú\par}
{\large 2026\par}
\end{titlepage}
...
El presente trabajo amplía esos contenidos sin apartarse del alcance introductorio de la unidad. El marco teórico se organiza en doce núcleos articulados: concepto de algoritmo; entrada y salida; propiedades; tipos primitivos; fundamentos de los TAD; interfaz e implementación; ejemplos de TAD; corrección; corrección parcial y total; lógica de Hoare; invariantes; y costo computacional con modelo RAM. La teoría se conecta con las implementaciones ya verificadas en Python y C++, con la prueba común de 24 elementos y con la comparación entre un recorrido lineal y dos ciclos anidados.
...
El marco se divide en doce núcleos para que la exposición grupal conserve una secuencia lógica y cada integrante pueda dominar un bloque coherente. La distribución propuesta no divide artificialmente el tema: los primeros cuatro núcleos establecen qué es un algoritmo y con qué datos trabaja; los cuatro siguientes estudian la abstracción y la corrección; y los cuatro últimos formalizan garantías y costo. Los límites entre bloques sirven para organizar la sustentación, pero las relaciones entre todos ellos se explican de forma explícita \citep{zanabria2026,cormen2022,wirth1976}.

\begin{table}[htbp]
\centering
\caption{Organización de los doce núcleos para la sustentación grupal}
\begin{tabularx}{\textwidth}{>{\bfseries}p{3.2cm}p{3cm}X}
\toprule
Integrante & Núcleos & Dominio principal\\
\midrule
Carlos Benedicto Yana Mendoza & 1--4 & Algoritmo, especificación, propiedades y tipos primitivos.\\
David Victor Belizario Yana & 5--8 & TAD, interfaz, implementaciones, ejemplos y corrección.\\
Handdy Ronald Curasi Zevallos & 9--12 & Corrección parcial y total, Hoare, invariantes y modelo RAM.\\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Núcleo 1: concepto de algoritmo y resolución de problemas}
...
La generalidad es otra consecuencia importante del concepto. El algoritmo para hallar el máximo no se diseña únicamente para el arreglo utilizado en la prueba, sino para cualquier arreglo que cumpla la precondición. Los 24 elementos permiten observar una ejecución concreta, mientras la definición y la demostración se formulan con $n$ para abarcar una familia ilimitada de instancias. Esta separación entre ejemplo e instancia general evita confundir una prueba exitosa con una garantía universal \citep{aho1974,cormen2022,knuth1997}.

Un algoritmo tampoco queda definido por la velocidad de una ejecución particular. Dos programas pueden implementar el mismo procedimiento en lenguajes o equipos diferentes y tardar tiempos físicos distintos, aun cuando realicen el mismo patrón lógico de comparaciones. Por ello, primero se identifica la estrategia y después se estudian su corrección y su costo bajo un modelo explícito \citep{aho1974,sedgewick2011,skiena2020}.

\subsection{Núcleo 2: entrada, salida y especificación del problema}
...
Definir el dominio de entrada permite decidir qué casos pertenecen al problema. En esta práctica se incluyen valores positivos, cero y negativos; la posición del máximo no está fijada y podrían existir valores repetidos. En cambio, un arreglo vacío queda fuera del dominio porque no contiene un valor que pueda satisfacer la salida. Esta decisión no es un detalle de programación, sino parte de la especificación que después aparece como precondición \citep{hoare1969,apt2009,cormen2022}.

La salida puede expresarse informalmente como ``el mayor valor'', pero para demostrar corrección conviene precisar dos condiciones: el resultado pertenece al arreglo y ningún elemento lo supera. La primera impide devolver un número ajeno a los datos; la segunda establece su maximalidad. Esta formulación muestra cómo una frase cotidiana se transforma en una postcondición verificable \citep{hoare1969,apt2009,dijkstra1975}.

\subsection{Núcleo 3: características y propiedades de un algoritmo}
...
Las propiedades pueden utilizarse como una lista de control al revisar una solución. Se pregunta si todas las entradas válidas están descritas, si la salida tiene una condición comprobable, si cada instrucción posee una interpretación única, si las operaciones son realizables y si existe un argumento de terminación. Aplicar estas preguntas al máximo revela de inmediato por qué se requiere un arreglo no vacío y por qué el índice debe aumentar en cada vuelta \citep{zanabria2026,aho1974,cormen2022}.

También es útil distinguir una propiedad del algoritmo de una propiedad de una sola ejecución. Observar que el programa terminó para $n=24$ no demuestra finitud para todo $n$; la garantía procede de la estructura del ciclo y del avance acotado del índice. Del mismo modo, acertar el máximo de un arreglo no prueba corrección general. Esta diferencia justifica combinar experimentación con razonamiento formal \citep{apt2009,hoare1969,cormen2022}.

\subsection{Núcleo 4: tipos de datos primitivos y representación}
...
La elección del tipo influye en qué entradas son representables. Un entero de C++ posee un rango finito dependiente de su tipo concreto, mientras que el entero de Python puede aumentar su tamaño interno según el valor disponible en memoria. Para la prueba se eligieron números pequeños, de modo que la comparación sea directa en ambos lenguajes y el análisis introductorio pueda tratarlos como valores de una palabra sin introducir costos por longitud numérica \citep{goodrichcpp2011,goodrichpy2013,python2026}.

El tipo también protege operaciones inválidas y comunica intención. Declarar un vector de enteros señala que sus elementos comparten un dominio comparable; usar un booleano para la verificación expresa que solo interesa saber si dos resultados coinciden. Antes de construir un TAD complejo, comprender estas garantías básicas permite reconocer qué aporta el lenguaje y qué reglas deben ser diseñadas por el programador \citep{wirth1976,goodrichcpp2011,sedgewick2011}.

\subsection{Núcleo 5: fundamentos de los tipos abstractos de datos}
...
La especificación de un TAD puede entenderse como un conjunto de valores válidos más reglas sobre sus operaciones. Por ejemplo, si se apila $x$ en una pila y no ocurre otra modificación, consultar el tope debe devolver $x$. Esta regla describe comportamiento sin indicar si $x$ se guardó en una posición de arreglo o en un nodo. La abstracción, por tanto, conserva lo esencial y pospone decisiones que no necesita conocer el usuario \citep{liskov1974,wirth1976,goodrichpy2013}.

Los TAD conectan algoritmos y estructuras de datos. Un algoritmo solicita operaciones abstractas, pero el costo real de atenderlas depende de la representación elegida. Esta relación explica por qué ambas materias se estudian juntas: la abstracción permite razonar con claridad y la implementación permite evaluar recursos. Confundirlas impediría cambiar una representación sin reescribir toda la solución \citep{wirth1976,aho1974,sedgewick2011}.

\subsection{Núcleo 6: diferencia entre interfaz e implementación}
...
La ocultación de detalles reduce el acoplamiento entre módulos. Si el código cliente accede directamente a posiciones internas, cualquier cambio de representación puede romperlo; si utiliza únicamente la interfaz, la implementación conserva libertad para mejorar. Esta idea es relevante incluso en un ejercicio pequeño: la función de máximo necesita recibir una colección recorrible y comparable, pero no necesita conocer cómo se administra su memoria internamente \citep{liskov1974,goodrichcpp2011,wirth1976}.

Una interfaz bien definida incluye casos de error. Una operación \codigo{desapilar} debe indicar qué ocurre cuando la pila está vacía; de modo semejante, la función del máximo debe declarar que requiere al menos un elemento. Explicitar estas condiciones evita que la implementación tome decisiones silenciosas y permite que precondiciones y excepciones reflejen el mismo contrato \citep{liskov1974,apt2009,goodrichpy2013}.
...
\subsection{Núcleo 7: ejemplos fundamentales de TAD}
...
Comparar los ejemplos revela que una colección no se identifica únicamente por sus elementos. Pila y cola podrían contener la misma secuencia, pero difieren en cuál elemento puede retirarse primero; lista y conjunto difieren en orden y duplicados; diccionario agrega una asociación entre clave y valor. Las operaciones permitidas y sus reglas observables son las que determinan el TAD \citep{sedgewick2011,goodrichpy2013,wirth1976}.

Para seleccionar un TAD conviene partir de las necesidades del problema y no de la estructura conocida por el programador. Si se requiere atender elementos en orden de llegada, una cola expresa directamente la política; si se requiere revertir acciones, una pila es más natural. La elección conceptual precede a decidir entre arreglo, lista enlazada u otra representación concreta \citep{skiena2020,aho1974,goodrichcpp2011}.

\subsection{Núcleo 8: especificación y corrección de algoritmos}
...
La corrección es relativa a la especificación, no a la intención no escrita del autor. Si la postcondición solo exigiera devolver un elemento del arreglo, casi cualquier posición cumpliría, pero no se habría formalizado el máximo. Una especificación débil puede hacer que un programa incorrecto parezca correcto; una especificación contradictoria puede ser imposible de satisfacer. Revisar el contrato es, por ello, parte de la verificación \citep{hoare1969,apt2009,dijkstra1975}.

Las pruebas con casos borde siguen siendo valiosas aunque no constituyan una demostración. Conviene incluir un único elemento, números negativos, máximos al principio y al final, y valores repetidos. Estos casos pueden descubrir fallos de implementación; la prueba formal explica después por qué, si se respetan las condiciones, el razonamiento cubre todos ellos \citep{cormen2022,apt2009,goodrichpy2013}.

\subsection{Núcleo 9: corrección parcial, corrección total y terminación}
...
La terminación suele justificarse mediante una función variante: una cantidad entera no negativa que disminuye estrictamente cada vez que el ciclo se repite. No basta con decir que ``el índice aumenta'', porque debe relacionarse ese aumento con un límite. En la búsqueda del máximo, $n-i$ comienza con $n-1$, disminuye en uno y llega a cero; por ello solo pueden ocurrir $n-1$ iteraciones \citep{apt2009,dijkstra1975,cormen2022}.

Separar parcial y total mejora la claridad de la prueba. Primero se demuestra que el estado conservado por el ciclo conduce a la postcondición; después se demuestra que el ciclo alcanza su salida. Esta división permite localizar errores: un invariante equivocado afecta la validez del resultado, mientras una variante que no disminuye afecta la garantía de terminación \citep{apt2009,hoare1969,dijkstra1975}.

\subsection{Núcleo 10: lógica de Hoare, precondición y postcondición}
...
La tripleta no afirma por sí sola que $P$ o $Q$ sean adecuadas: estas deben describir fielmente el problema. En el máximo, $P$ incluye la existencia de al menos un elemento y $Q$ incluye pertenencia y maximalidad. Una vez fijadas, cada línea del algoritmo puede interpretarse como una transformación de estado que acerca $P$ hacia $Q$ sin perder las propiedades necesarias \citep{hoare1969,apt2009,cormen2022}.

Para una estructura condicional se consideran los dos caminos. Si $A[i]$ supera al máximo anterior, la asignación incorpora el nuevo valor; si no, conservar el anterior es correcto. El análisis de ambas ramas evita razonar solo sobre el caso en que existe actualización y demuestra que la postcondición intermedia se mantiene cualquiera que sea el resultado de la comparación \citep{hoare1969,dijkstra1975,apt2009}.

\subsection{Núcleo 11: invariantes de ciclo e inducción}
...
El razonamiento del invariante se parece a una inducción. El inicio cumple el papel del caso base; la preservación demuestra que la verdad pasa de un prefijo al siguiente; y la finalización aplica la propiedad al prefijo que ya coincide con el arreglo completo. Esta analogía ayuda a comprender por qué tres ejemplos consecutivos no bastan: la preservación debe formularse para una iteración arbitraria \citep{cormen2022,apt2009,hoare1969}.

Un buen invariante debe ser suficientemente fuerte para implicar la postcondición al finalizar, pero también demostrable desde la inicialización. Decir solamente que \codigo{maximo} pertenece al arreglo es verdadero, pero no prueba que sea el mayor; afirmar desde el inicio que ya es el máximo global sería fuerte, pero no está establecido. El máximo del prefijo logra el equilibrio correcto \citep{apt2009,cormen2022,dijkstra1975}.

\subsection{Núcleo 12: costo computacional, modelo RAM y crecimiento}
...
El conteo exacto solo tiene sentido acompañado por su convención. Tratar un \codigo{for} como una unidad produce una expresión distinta de expandir inicialización, condición e incremento; contar accesos a memoria también agrega términos. Lo metodológicamente correcto es declarar el modelo, aplicar la misma granularidad a los algoritmos comparados y separar la fórmula exacta de la clase de crecimiento \citep{cook1973,aho1974,cormen2022}.

El tamaño $n$ funciona como puente entre programa y función matemática. Para el máximo representa elementos; en los ciclos anidados sigue representando el límite de cada recorrido. Una vuelta completa genera un término lineal, mientras repetirla para cada valor exterior produce el producto $n\cdot n$. La fórmula conserva el detalle del conteo y el término dominante resume la tendencia al crecer la entrada \citep{cormen2022,sedgewick2011,knuth1976}.

La conclusión integradora es que corrección y costo contestan preguntas diferentes pero complementarias. El invariante demuestra que \codigo{encontrar\_maximo} devuelve el valor debido; el modelo RAM muestra que lo consigue con crecimiento lineal. En el fragmento anidado, el conteo explica por qué el trabajo dominante es cuadrático, independientemente de que el equipo ejecute cada operación con mayor o menor rapidez \citep{hoare1969,cook1973,skiena2020}.
...
\bibitem[Aho et al.(1974)]{aho1974}
Aho, A. V., Hopcroft, J. E., \& Ullman, J. D. (1974). \textit{The design and analysis of computer algorithms}. Addison-Wesley.

\bibitem[Apt et al.(2009)]{apt2009}
Apt, K. R., de Boer, F. S., \& Olderog, E.-R. (2009). \textit{Verification of sequential and concurrent programs} (3.ª ed.). Springer. \url{https://doi.org/10.1007/978-1-84882-745-5}
...
\bibitem[Wirth(1976)]{wirth1976}
Wirth, N. (1976). \textit{Algorithms + data structures = programs}. Prentice-Hall.
